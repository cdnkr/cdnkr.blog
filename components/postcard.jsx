  ...
  return (
    <Link href={`/post/${slug}`}>
      <Card
        ...
        style={{
          color: isFullyVisible ? "var(--color-primary)" : "rgba(var(--color-dark-rgb), 0.7)",
        }}
        {...rest}
      >
      ...
      <div className="font-mono text-xs mt-1">{post.frontmatter.date.toISOString()}{" "}{debug?.top?.toFixed()}{" "}{debug?.left?.toFixed()}{" "}{debug?.bottom?.toFixed()}{" "}{debug?.right?.toFixed()}{" "}{debug?.viewportHeight}{" "}{debug?.viewportWidth}{" "}{debug?.scrollY?.toFixed()}{" "}{isFullyVisible ? "true" : "false"}</div>
    </Link>
  );
};
