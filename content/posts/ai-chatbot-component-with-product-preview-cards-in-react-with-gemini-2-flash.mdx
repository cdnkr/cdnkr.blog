...
<Block
  className="not-prose text-tertiary mt-6"
  variant="dark"
  caretPosition="top-left"
>
  <h3 className="text-xl uppercase font-bold">Please note</h3>
  <p>
    This is not the recommended implementation, as it exposes environment keys
    client side.
    <br />
    While this tutorial details how to restrict your API key to only your site,
    the Next.js implementation is favored as API keys are never exposed to the
    client. See{" "}
    <a
      href="/post/ai-chatbot-component-with-product-preview-cards-in-next-with-gemini-2-flash"
      className="uppercase underline font-bold"
    >
      this example
    </a>{" "}
    for the full Next.js approach.
  </p>
</Block>
---
