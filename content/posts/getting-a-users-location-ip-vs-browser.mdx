...
The technical test for my first job as a junior developer involved showing a user's location on a webpage. I decided to use Mapbox to visualize that data on a map.  I even made it so the marker would update as they moved around. It was pretty cool seeing something I built actually *do* something in the real world. I even went for a ride around my neighbourhood watching the marker move. I got the job, so it must have been decent enough.

_Quick side note:_ I did all this on a super old 3rd gen i3 laptop. Point is, getting into web dev doesn't require a crazy powerful machine, especially for front-end stuff.
